...
trait PageFormTrait
{
        ...
        return $form
            ->schema([
                    ...
                    ->schema([
                        Section::make()
                            ->schema([
                                TextInput::make('name')
                                    ->required()
                                    ->live(debounce: config('filament.debounce_time'))
                ...
                Group::make()
                    ->schema([
                        Section::make('Status')
                            ->schema($this->statusFields()),
                        Section::make('Related pages')
                            ->schema($this->relatedPagesActions()),
                    ])
                    ->columnSpan(['lg' => 3]),
            ])->columns(12);
    }
    ...
    protected function relatedPagesActions(): array
    {
        return [
            Actions::make([
                Action::make('manage')
                    ...
                    ->form([
                        Repeater::make('related_pages')
                            ->schema([
                                Select::make('locale')
                                    ->options(Language::query()->pluck('name', 'locale'))
                                    ->required(),

                                Select::make('page_id')
                                    ->label('Page')
                                    ->options(Page::query()->pluck('name', 'id'))
                                    ->searchable()
                                    ->required(),
                            ])
                            ->columns()
                    ...
                    ->action(function (array $data, Page $record): void {
                        $record->related_pages = $data['related_pages'];
                        $record->save();
                    })
            ])
        ];
